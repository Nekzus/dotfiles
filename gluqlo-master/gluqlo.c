...
const int DEFAULT_WIDTH = 1920;	 /*1024*/
const int DEFAULT_HEIGHT = 1080; /*768*/
...
void render_clock(int maxsteps, int step)
{
	...
	if (_time->tm_hour != past_h)
	{
		...
		// draw am/pm
		if (!twentyfourh)
			render_ampm(screen, &hourBackground, _time->tm_hour >= 12);
	}
...
int main(int argc, char **argv)
{
	...
	double display_scale_factor = 1.35;
	...
	if (is_horizontal)
	{
		hourBackground.x = 0.5 * (width - (0.031 * width) - (1.2 * height)) + jitter_width;
	...
	{
		hourBackground.y = 0.5 * (height - (0.031 * height) - (1.2 * width)) + jitter_height;
